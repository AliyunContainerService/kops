...
const (
	typeAutoScalingGroup  = "AutoScalingGroup"
...
type clusterDiscoveryALI struct {
	...
	clusterName      string
	autoScalingGroup map[string]string
}
...
func ListResourcesALI(aliCloud aliup.ALICloud, clusterName string, region string) (map[string]*resources.Resource, error) {
	...
	for _, t := range resourceTrackers {
		d.autoScalingGroup[t.Name] = t.ID
...
func (d *clusterDiscoveryALI) ListAutoScalingGroups() ([]*resources.Resource, error) {
...
func (d *clusterDiscoveryALI) ListLoadBalancer() ([]*resources.Resource, error) {
	...
	if len(loadBalancers) > 1 {
		return nil, fmt.Errorf("found multipe LoadBalancers with the same name: %v", loadBalancerName)
	}
...
func DeleteNatGateway(cloud fi.Cloud, r *resources.Resource) error {
	c := cloud.(aliup.ALICloud)
	glog.V(2).Infof("Removing NatGateway with Vpc Id %s", r.ID)

	ngs, _, err := c.VpcClient().DescribeNatGateways(&ecs.DescribeNatGatewaysArgs{VpcId: r.ID})

	if err != nil {
		return fmt.Errorf("err list NatGateway:%v", err)
	}
	if ngs == nil || len(ngs) == 0 {
		return nil
	}

	for _, ng := range ngs {
		err := c.VpcClient().DeleteNatGateway(&ecs.DeleteNatGatewayArgs{NatGatewayId: ng.NatGatewayId})
		if err != nil {
...
func (d *clusterDiscoveryALI) ListSecurityGroup() ([]*resources.Resource, error) {
	...
	if len(groupTrackers) != 0 {
		for _, groupTracker := range groupTrackers {
			for autoScalingGroupName, autoScalingGroupId := range d.autoScalingGroup {
				if strings.Contains(autoScalingGroupName, groupTracker.Name) {
					groupTracker.Blocked = append(groupTracker.Blocked, typeAutoScalingGroup+":"+autoScalingGroupId)
				}
			}
...
func (d *clusterDiscoveryALI) ListVPC() ([]*resources.Resource, error) {
	...
	if len(vpcsToDelete) > 1 {
		...
		for _, vswitchId := range vswitchsToDelete {
			...
			for _, autoScalingGroupId := range d.autoScalingGroup {
				resourceTracker.Blocked = append(resourceTracker.Blocked, typeAutoScalingGroup+":"+autoScalingGroupId)
			}
			resourceTrackers = append(resourceTrackers, resourceTracker)
		}
	}
