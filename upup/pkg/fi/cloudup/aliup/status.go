...
func buildCloudInstanceGroup(c ALICloud, ig *kops.InstanceGroup, g ess.ScalingGroupItemType, nodeMap map[string]*v1.Node) (*cloudinstances.CloudInstanceGroup, error) {
	...
	for _, i := range instances {
		//Hostname and instanceId are not exactly the same, need to be adjusted.
		instanceId := i.InstanceId
		b := []byte(instanceId)
		b = append(b, 'z')
		if len(b) < 2 {
			glog.Warningf("The length of instanceid is less than 2: %s", i)
			continue
		}
		b[1] = 'z'
		instanceId = string(b)
...
// DeleteInstance deletes an ali instance
func (c *aliCloudImplementation) DeleteInstance(i *cloudinstances.CloudInstanceGroupMember) error {
	...
	b = append(b[:len(b)-1])
	if len(b) < 2 {
		return fmt.Errorf("The length of instanceid is less than 2: %s", i)
	}
	b[1] = '-'
	id = string(b)
	...
	if err := c.WaitForInstanceStopped(id); err != nil {
		return err
	}
	...
	glog.V(8).Infof("deleted ali ecs instance %q", id)
	return nil
}

func (c *aliCloudImplementation) WaitForInstanceStopped(instanceId string) error {
	instanceIds := []string{}
	instanceIds = append(instanceIds, instanceId)

	id, err := json.Marshal(instanceIds)
	if err != nil {
		fmt.Println(err)
	}
	...
	for attempt := 1; ; attempt++ {
		...
		instances, _, err := c.EcsClient().DescribeInstances(&describeInstancesArgs)
		if err != nil {
			return fmt.Errorf("error waitting for instance to stop,instanceId %q: %v", instanceId, err)
		}

		if len(instances) != 1 {
			return errors.New("error waitting for instance to stop, more than one machine returned.")
		}

		if instances[0].Status == ecs.Stopped {
			return nil
		}

		if attempt == MaxWaitTime {
			return errors.New("error waitting for instance to stop, exceeded wait time")
		}
